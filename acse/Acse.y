...
declaration : IDENTIFIER ASSIGN NUMBER
            ...
            {
                /* same as alloc_declaration but with
                   isMatrix, rows and columns
                   parameters at the end */
                $$ = matrix_alloc_declaration($1, 0, 0, 0, 1, $3, $5 );
...
assign_statement : IDENTIFIER LSQUARE exp RSQUARE ASSIGN exp
            ...
            {
                //idx = row + col * l. column
                t_axe_variable *m = getVariable(program, $1);
                int pos = $3.value + $5.value * (m->columns) ;
                int idx = create_expression(pos, IMMEDIATE);
                storeArrayElement(program, $1, idx, $8);
...
exp: NUMBER      { $$ = create_expression ($1, IMMEDIATE); }
   ...
   | IDENTIFIER LSQUARE exp COMMA exp RSQUARE {
       ...
       t_axe_variable *m = getVariable(program, $1);
       int pos = $3.value + $5.value * (m->columns);
       t_axe_expression idx = create_expression(pos, IMMEDIATE);
       reg = loadArrayElement(program, $1, idx);
