...
assign_statement : IDENTIFIER LSQUARE exp RSQUARE ASSIGN exp
            ...
            | IDENTIFIER AR_COPY IDENTIFIER
            {
                ...
                int i;
                for(i = 0; i < v1->arraySize; i++)
                {
                    t_axe_expression idx = create_expression(i, IMMEDIATE);
                    t_axe_expression data;
                    if(i < v2->arraySize)
                    {
                        //copy $3[i]
                        int newData = loadArrayElement(program, $3, idx);
                        data = create_expression(newData, REGISTER);
                    }
                    else
                    {
                        //assign 0
                        data = create_expression(0, IMMEDIATE);
                    }
                    storeArrayElement(program, $1, idx, data);
                }
                free($1);
                free($3);
            }
